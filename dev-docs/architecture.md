...
`agent.continue`, `agent.pause`, `agent.resume`, `agent.stop`, `agent.cancel`, and `agent.restart` now execute through the server-side command worker. `agent.pause` and `agent.resume` call explicit `AgentAdapter.pause/resume` methods; they must not be implemented as magic text sent through `sendMessage`. The Codex adapter starts a session in `waiting_for_user`, then `agent.continue` launches one non-interactive `codex exec --json` process with the queued prompt. When Codex emits `thread.started`, the adapter stores the provider `thread_id` in its session handle and later Continue commands use `codex exec resume --json <thread_id> <prompt>` so the same logical Agent session keeps conversational context while the daemon stays alive. `agent.restart` stops the current provider handle, keeps the same logical Session, clears stale terminal timestamps/errors, starts the adapter again with the persisted workspace/agent identity, and writes the new Runtime status/pid back to SQLite. `agent.screenshot`, `workspace.test.run`, and `workspace.deploy.run` are reserved command types and fail explicitly until their handlers are implemented. HTTP routes must not bypass the queue for new control actions.
