...
- 正式名称：**NCC AI Development OS**。
- 简称：**NCC AI OS**。
- 禁止定位：不要把本项目称为或设计成 `Codex Dashboard`。
- 长期边界：它管理的是 AI Agent、GitHub 项目、部署、日志监控、远程控制、知识库和自动化工作流。
- 当前 Sprint 边界：先完成 Agent Runtime 闭环；在真实 Telegram 端到端验收之前，不开发 Plugin、MCP 或新的 Dashboard 能力。
...
- Codex `exec --json` 进程适配器和空跑智能体适配器。
- Command Queue 基线：Continue、Pause、Resume、Stop、Cancel、Restart 统一入队后执行。
- Agent Runtime 持久状态：运行中 pid、Heartbeat、终态清理、Server 关闭时收束 Agent 子进程。
